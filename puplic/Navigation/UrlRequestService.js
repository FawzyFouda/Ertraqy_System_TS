...
export class UrlRequestService {
    ...
    // ___________________________________________________________
    static contentDrawPostForm(contentDrawPostFormOptions) {
        var modalsOpened = {};
    ...
    ;
    // ___________________________________________________________
    static _tableFixedHeaderSet(options) {
        // console.log(options)
        if (options == undefined)
            options = {};
        if (options.divParentSelector == undefined)
            options.divParentSelector = 'div.table-fixed-header';
        if (options.tableSelector == undefined)
            options.tableSelector = options.divParentSelector + ' > table:visible'; // table.fixed-header:visible
        if (options.clearWidth == undefined)
            options.clearWidth = true;
        if (options.mainHeight == undefined || options.mainHeight <= 0)
            options.mainHeight = $(window).height();
            ...
            if (columns > 0) {
                for (let i = 1; i <= columns; i++) {
                    ...
                    if (tableVertical)
                        ...
                        cell.css('width', maxWidth);
                    if (cell.hasClass('col-sticky')) {
                        if (tableVertical)
                            cell.css('top', heightInRow);
                        else
                            cell.css('right', widthInRow);
                        stickyFound = true;
                    }
    ...
    // ___________________________________________________________
    static staticorderCalculateWidths(addNew) {
    ...
    // ___________________________________________________________
    static addEditItem_Success(options, data) {
        $('.print').removeAttr('disabled');
        $('.print-receipt').prop('disabled', false);
        $('.table-columns-icon').removeClass('disabled');
        var dataHeader, dataRow, dataFooter, dataSave = null;
        ...
        if (invoiced == 'False' || invoiced == '')
            invoiced = '0';
        else
            invoiced = '1';
        var delivered = $(dataHeader).attr('delivered');
        if (delivered == 'False' || delivered == '')
            delivered = '0';
        else
            delivered = '1';
    ...
    // ___________________________________________________________
    static excuteAfterAddEditItemsInOrder() {
        this.formReset;
    ...
    ;
    // ___________________________________________________________
    static addItemsClickRev2(product_id_rev2) {
        ...
        var contact = this._getUrlQueryStrings()["contact"];
        ...
        var urlPrefix = this._urlWithHash();
    ...
    ;
    // ___________________________________________________________
    static sideBarLeftLoad(options) {
    ...
    ;
    // ___________________________________________________________
    static contentDraw(contentOptions) {
        // contentOptions.Url
    ...
    // ___________________________________________________________
    static loadPaymentsOrDuesListSideBar(event) {
    ...
    // ___________________________________________________________
    static _getDynamicUrlQueryStrings(_url) {
        var vars = [], hash;
    ...
    ;
    // ___________________________________________________________
    static createNewBillClick(clickStatus, transId) {
    ...
    ;
    // ___________________________________________________________
    static modalSidebarClose(selector) {
    ...
    // };
    // ___________________________________________________________
    static ertaqyLiveChatPopupHide() {
    ...
    ;
    // ___________________________________________________________
    static getUrlForPushState(url, title) {
        // current url
        var paths = window.location.pathname.split('/');
        if (paths[0] == '')
            paths.splice(0, 1);
    ...
    // }
    // ____________________________________________________________________________
    static _signalrSrvSendJson(json) {
        if (this._signalrHub1) {
            try {
                ...
                if (json.await == true) {
                    this._signalrHub1.server.send(JSON.stringify(json))
                        .done(() => {
                        if (json.callback != null && json.callback != undefined) {
                            json.callback(json);
                        }
                        if (this._signalrLogging)
                            console.log("signalR Sent: " + JSON.stringify(json));
                    })
                        .fail(() => {
                        if (json.callback != null && json.callback != undefined) {
                            json.callback(json);
                        ...
                        if (this._signalrLogging)
                            console.log("signalR Failed: " + JSON.stringify(json));
                    });
                }
                else {
                    this._signalrHub1.server.send(JSON.stringify(json))
            ...
            if (this._signalrLogging)
                console.log('signalr Hub is null');
        }
    }
    // ____________________________________________________________________________
    static _ertaqyAjax(options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (options.requestKey == undefined)
                options.requestKey = 'global-request';
            ...
            if (options.type == 'POST' && options.formBtn) {
                ...
                if (options.requestKey)
                    ajaxReqPrev = this._ertaqyAjaxRequests[options.requestKey];
                ajaxReqNew = $.ajax({
                    type: options.type,
                    cache: options.cache,
                    async: options.async,
                    ...
                    beforeSend: function (xhr) {
                        ...
                        this._ertaqyAjaxCounts++;
                        if (btn)
                            setTimeout(function () { btn.attr("disabled", "false"); }, 2000); // allow button again if loading takes time
                        // Stop Previous Request with same requestKey
                    ...
                    if (options.requestKey)
                        delete this._ertaqyAjaxRequests[options.requestKey]; // no need to call to abort again
                });
                // current request, to be checked next request if doublicated, before await when
                if (options.requestKey)
                    this._ertaqyAjaxRequests[options.requestKey] = ajaxReqNew; // set current new ajaxRequest to allow abort in next call
                // await here instead of await ajax, to check dublicates in beforeStart
                yield $.when(ajaxReqNew);
            }
        });
    }
    // _______________________________close all --- click Arrow_____________________________________________
    static sideBarsclose(selector) {
    ...
    // ____________________________________________________________________________
    static setActiveToLink(selector, container) {
    ...
    static showTickets() {
        ...
        var taskId = _a._uuidv4();
        var templateId = $(this).closest('tr').attr('template-id');
        var typeparentId = $(this).closest('tr').attr('type-id');
        _a.openTicketDetailsSideBar(ticketNum, ticketId, contactId, contactName, taskId, templateId, typeparentId);
    }
    // ____________________________________________________________________________ 
    static Create_Layout_Sidebar(options) {
    ...
    // ____________________________________________________________________________ 
    static openModalSidebar(options) {
        if (options.modalKey)
        ...
        if ($('#' + options.ModalId + ' .modal-sidebar-content').html() != '') {
            ...
            if (options.sideBarsList) {
                ...
                var ModalId_Selecteor = this._sideBarsOpenedArray.filter((e) => { var _b; return ((_b = $(`a[modal-id="${e}"]`)) === null || _b === void 0 ? void 0 : _b.text()) === options.ModalName; });
                //get parent of <a> tickit (li) and set its class to active
                $(`a[modal-id="${ModalId_Selecteor}"]`).parent().addClass("active");
                let Not_ModalId_Selecteor = this._sideBarsOpenedArray.filter((e) => { var _b; return ((_b = $(`a[modal-id="${e}"]`)) === null || _b === void 0 ? void 0 : _b.text()) !== options.ModalName; });
                Not_ModalId_Selecteor.forEach((element) => {
            ...
            if (options.selecElement.attr('action-mode') !== "sidebar1" || options.selecElement.attr('action-mode') == undefined) {
                ...
                this._sideBarsOpenedArray = this._sideBarsOpenedArray.filter((value, index, self) => {
            ...
            if (options.url) {
                ...
                this.Request(options);
                options.done();
                options.always = function (alwaysOptions) {
    ...
    // ____________________________________________________________________________
    static _drawInContentSidebar(options) {
        options.type = "POST";
    ...
    // ____________________________________________________________________________
    // ____________________________________________________________________________
    static openTicketDetailsSideBar(ticketNum, ticketId, contactId, contactName, taskId, templateId, typeparentId) {
        ...
        urlRequestOptions.done = function () {
            ...
            (_c = (_b = urlRequestOptions.Container) === null || _b === void 0 ? void 0 : _b.ParentSelector) === null || _c === void 0 ? void 0 : _c.html('').append(urlRequestOptions.paramData);
            $('.content-wrap').attr('current-page-url', urlRequestOptions.url);
        };
        this.openModalSidebar(urlRequestOptions);
    }
}
_a = UrlRequestService;
UrlRequestService._ertaqyModalsOpened = {};
UrlRequestService._ertaqyAjaxRequests = {};
UrlRequestService._ertaqyAjaxCounts = 0;
UrlRequestService._ertaqyAjaxAlertSlowMsg = null;
UrlRequestService.openModalSidebarModals = {};
UrlRequestService._sideBarsOpenedArray = [];
UrlRequestService._signalrLogging = false;
UrlRequestService._signalrConnGroups = [];
UrlRequestService.addEditItem_InProgress = false;
UrlRequestService.dataSplitCheck = false;
UrlRequestService._tableFixedHeaderSets = {};
// ___________________________________________________________
UrlRequestService.formReset = function () {
    document.getElementById('a79a2cdd-4e9c-4187-acbc-1e1d2ac397bf').reset();
    //$('.tt-input').val('');
    $('.code-input').focus();
};
// ___________________________________________________________
UrlRequestService.setBillFooterTotalInput = function () {
...
// ___________________________________________________________
UrlRequestService.afterLoadPaymentsOrDuesListSideBar = function (options) {
...
// ___________________________________________________________
UrlRequestService.Request = (options) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (options.url.endsWith('?'))
        options.url = options.url.substring(0, options.url.length - 1);
    if (_a._getUrlQueryStrings()['sqltrace'] == '1' && options.url.indexOf('sqltrace=') == -1)
        options.url += (options.url.indexOf('?') == -1 ? '?' : '&') + 'sqltrace=1';
    if (options.success == undefined)
        options.success = options.done; // till change all done to success
    if (options.resultFuncAwait == undefined)
            ...
            if ($form.find('.input-html-group').length > 0) {
                $form.find('.input-html-group input, .input-html-group textarea').each(() => {
                    if ($(_a).val() != '') {
                        //console.log('Encoding -------------------------');
                        //console.log(_htmlEncode($(this).val()));
                        $(_a).val(String($(_a).val()).EncodeHtml());
                    }
                });
            }
            let jsonData = _a.getFormData($form);
            btn.attr("disabled", "true");
            // Reinitialize Validation because form added dynamically
            $form.removeData('validator');
            $form.removeData('unobtrusiveValidation');
                ...
                if ($form.find('.validSummery').length > 0) {
                    ...
                    $form.find('.field-validation-error').each(() => {
                        if ($(_a).find('span').length > 0) {
                            var inputValidTxt = $(_a).parent().find('label').text().replace(':', '');
                            $form.find('.validSummery').append('<div>* الرجاء ادخال ' + inputValidTxt + '</div>');
                        }
                    });
                }
    ...
    var ajaxReqPrev;
    if (options.requestKey)
        ajaxReqPrev = _a._ertaqyAjaxRequests[options.requestKey];
    var ajaxType;
    switch (options.Method) {
        case UrlRequestMethods.Get: {
            ajaxType = "GET";
            break;
        }
        case UrlRequestMethods.Post: {
            ajaxType = "POST";
            break;
        }
    }
    var ajaxReqNew = $.ajax({
        ...
        beforeSend: function (xhr) {
            ...
            // this._ertaqyAjaxCounts++;
            if (btn)
                setTimeout(() => { btn.attr("disabled", "false"); }, 2000); // allow button again if loading takes time
            // Stop Previous Request with same requestKey
            if (this.ajaxReqPrev != undefined && this.ajaxReqPrev.readyState < 4) {
                this.ajaxReqPrev.abort();
                $('#ertaqy-chat-loader').length == 1 ? $('#ertaqy-chat-loader').fadeIn() : $('#ajax-loader').fadeIn();
        ...
        if (options.requestKey)
            delete _a._ertaqyAjaxRequests[options.requestKey]; // no need to call to abort again
    });
    // current request, to be checked next request if doublicated, before await when
    if (options.requestKey)
        _a._ertaqyAjaxRequests[options.requestKey] = ajaxReqNew; // set current new ajaxRequest to allow abort in next call
    // await here instead of await ajax, to check dublicates in beforeStart
    yield $.when(ajaxReqNew).promise(); //.done();
    console.log("5--fifth await $.when(ajaxReqNew).promise()");
    console.log("-------------------------------------End Request Function-------------------------------------------------");
});
// ____________________________________________________________________________
UrlRequestService._signalrSrvRemoveGroup = (groupName, type) => {
    console.log('_signalrSrvRemoveGroup --------------------' + groupName);
    if (_a._signalrHub1) {
        var result = _a._signalrHub1.server.removeGroup(groupName, type);
        delete _a._signalrConnGroups[groupName];
        if (_a._signalrLogging)
            console.log(result);
    }
    else {
        if (_a._signalrLogging)
